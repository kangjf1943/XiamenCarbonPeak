...
```{r ActivityLevel}
par(mfrow = c(3, 3))
func_history_project(agri_act[["BY"]], "agri", agri_act[["BAU_SLC_DECOAL_OTHER"]], "area", style = "base")
for (i in list(ind_act, const_act, trans_act, com_act, hh_act)) {
  for (j in names(i[["BY"]])[names(i[["BY"]]) != "year"]) {
    func_history_project(i[["BY"]], j, i[["BAU_SLC_DECOAL_OTHER"]], j, style = "base")
  }
}
```
...
for (i in list(ind_nrgintst)) {
  for (j in names(i[["BY"]])) {
    ...
    for (k in names(i[["BY"]][[j]])[names(i[["BY"]][[j]]) != "year"]) {
      plot(1, main = k)
      func_history_project(i[["BY"]][[j]], k, i[["BAU_SLC_DECOAL_OTHER"]][[j]], k, style = "base")
    }
  }
}
```
...
func_history_project(hh_nrgintst[["BY"]]$household, "electricity", hh_nrgintst[["BAU_SLC_DECOAL_OTHER"]]$hh_coal_elec, "electricity", style = "base")
plot(1, main = "lpg_users")
func_history_project(hh_nrgintst[["BY"]]$household, "rawcoal", hh_nrgintst[["BAU_SLC_DECOAL_OTHER"]]$hh_coal_elec, "rawcoal", style = "base")

plot(1, main = "lpg_users")
func_history_project(hh_nrgintst[["BY"]]$lpg, "lpg", hh_nrgintst[["BAU_SLC_DECOAL_OTHER"]]$hh_lpg, "lpg", style = "base")

plot(1, main = "gas_users")
func_history_project(hh_nrgintst[["BY"]]$gas, "gas", hh_nrgintst[["BAU_SLC_DECOAL_OTHER"]]$hh_gas, "gas", style = "base")
```
