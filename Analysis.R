...
# 目前可选的情景包括：
# c("BAU_KEEPCOAL", "BAU", "BAU_WLCMINUS_C2G", "BAU_WLC_C2G", "BAU_WLCMINUS", "BAU_WLC", "BAU_SLCPLUS", "BAU_SLC_DECOAL")
...
if (set_cache_globalvar == FALSE) {
  ...
  ## GDP ----
  # 规上工业各行业GDP
  global_indscale_gdp4sctr <- func_read_trans("工业各行业GDP和能耗", "工业GDP")
  ...
  # 全市各产业GDP
  global_gdp <- func_read_trans("厦门GDP和人口", "GDP")
  ...
  # 预测GDP相关项目变化
  prj_global_gdp <- func_read_trans("厦门市GDP预测")[1:16, ]
  ...
  prj_global_gdp$constgdp_prop <- 
    func_interp_2(
      year = c(2019, 2060), 
      value = c(global_gdp$constgdp_prop[global_gdp$year == 2019], 10), 
      showplot = set_showplot)$value
  ...
  ## Population ----
  global_population <- func_read_trans("厦门GDP和人口")
  ...
  prj_global_population <- func_interp_2(
    ...
    value = c(global_population$"常住人口"[global_population$year == 2019], 
              575, 650, 730, 780, 800), "population", showplot = set_showplot)
...
if (set_cache_readdata == FALSE) {
  # Read data ----
  # 读取并构建排放因子数据
  global_ori_emisfac_df <- func_read_data("排放因子")[global_nrg_class]
  ...
  # 农业柴油
  global_agri_diesel <- 
    func_read_trans("碳排放峰值模型参数选择检验")[, c("year", "农用柴油使用量")]
  
  # 生活原煤
  global_hh_coal <- func_read_trans("生活用煤")
  ...
  global_avnnrg <- func_read_trans("厦门市国内国际航空煤油消费量")
  # 补全包含福州机场消费量在内的总消费量
  ...
  # 读取航空客货运周转量
  global_avn_act <- 
    func_read_trans("航空客货运量及周转量")[c("year", "客运周转量", "货运周转量")]
  names(global_avn_act) <- c("year", "avn_rpk", "avn_rftk")
  
  # 读取水运客货运周转量
  global_water_act <- 
    func_read_trans("水路客货运量及周转量")[c("year", "客运周转量", "货运周转量")]
  names(global_water_act) <- c("year", "water_rpk", "water_rftk")
  
  # 读取园林局LPG数据
  global_ind_com_hh_lpg <- func_read_trans("测试用工业和服务业和生活LPG消费")
  ...
  # 读取园林局天然气数据
  global_com_hh_gas <- 
    func_read_trans("测试用市政园林局的生活和服务业天然气消费")
  ...
  # 读取交通天然气
  global_trans_gas <- 
    func_read_trans("营运性车辆活动水平和能耗", "天然气消费量")
  
  # 读取交通电力消费并合并地铁用电
  # 此处仅进入部分车型用电量
  # 问题：网约车用电量不低，但因无保有量数据，难以从私家车区分出来，暂不处理
  global_trans_elecsec <- 
    func_read_trans("营运性车辆活动水平和能耗", "电力消费")
  ...
  # 读取用电数据
  global_elecaggsec <- func_read_trans("厦门市电力数据", "全市电力消费情况表")
  global_elecfinesec <- 
    func_read_trans("厦门市电力数据", "全市电力消费情况表分具体行业")
  
  # 读取本地发电数据并计算清洁和非清洁发电比例
  global_elecgen <- func_read_trans("厦门市电力数据", "全市发电量")
  ...
  # 读取水运燃料油
  global_trans_residual <- 
    func_read_trans("测试用水运燃料油消耗量港口局数据推算")
  ...
  global_water_railway_diesel <- func_read_trans("测试用水运和铁路柴料油消耗量港口局数据推算")
  ...
  global_roadoper_diesel <- 
    func_read_trans("营运性车辆活动水平和能耗", "柴油消费量")[
      c("year","常规公交","BRT","农村客车")]
  
  # 读取慧梅推算数据：营运非营运车辆柴油
  global_roadnonoper_diesel <- 
    func_read_trans("非营运性车辆活动水平和能耗推算", 
                    "非营运性车辆柴油消费推算")
  ...
  global_solarelecgen_fut <- func_read_trans("厦门市太阳能发电量预测数据更新")
  
  # 读取全省发电量
  global_provelecgen <- 
    func_read_trans("福建省发电行业能源消费量和发电量", "发电量")
  
  # 读取外调电力折标煤系数
  global_importthrm_cefac <- func_read_trans("厦门市电力数据", "XCP外调电力")
...
if (set_cache_nrgbal == FALSE) {
  ...
  for (i in by_nrgbal_years) {
    # 发电量
    by_nrgbal_ls[[i]][which(
      by_nrgbal_ls[[i]]$iterm == "tf"), "electricity"] <- 
      global_elecgen[which(global_elecgen$year == i), "合计"]
    # 农业用电
    by_nrgbal_ls[[i]][which(
      by_nrgbal_ls[[i]]$iterm == "agri"), "electricity"] <- 
    ...
    # 交通用电量
    by_nrgbal_ls[[i]][which(
      by_nrgbal_ls[[i]]$iterm == "trans"), "electricity"] <- 
      global_trans_elecsec[which(
        global_trans_elecsec$year == i), "常规公交"] + 
      global_trans_elecsec[which(
        global_trans_elecsec$year == i), "纯电动出租车"] + 
      global_trans_elecsec[which(
        global_trans_elecsec$year == i), "地铁"]
    # 服务业用电
  ...
  # 读取统计局物理量总量数据
  by_checknrgaggfuel <- func_read_trans("全市总量数据整理")
  by_checknrgaggfuel[
  ...
  by_checknrgaggfuelce <- 
    func_read_trans("全市总量数据整理", "XCP_煤油气电标准量")
  by_checknrgaggfuelce[
    c("煤炭消费量", "油品消费量", "天然气消费量", "调入电力")] <- 
    by_checknrgaggfuelce[
      c("煤炭消费量", "油品消费量", "天然气消费量", "调入电力")]*10000
...
if (set_cache_hiscalc == FALSE) {
  ...
  # 农业的播种面积
  # 读取《碳排放峰值模型参数选择检验》
  agri_act[["BY"]] <- func_read_trans("碳排放峰值模型参数选择检验")
  agri_act[["BY"]] <- 
  ...
  # 营运车辆里程数
  by_trans_operation <- func_read_trans("营运性车辆活动水平和能耗", "里程数")
  by_trans_operation <- 
  ...
  by_trans_nonoperation <- 
    func_read_trans("非营运性车辆活动水平和能耗推算")[c("year", "#常规私家车", "#纯电动私家车")]
  ...
  by_trans_nrgfuel_ori[[1]] <- 
    func_read_trans(
      "营运性车辆活动水平和能耗", "汽油消费量")[, c("year", "出租车合计")]
  names(by_trans_nrgfuel_ori[[1]])<- c("year", "出租车")
  # 柴油消费量
  by_trans_nrgfuel_ori[[2]] <- global_roadoper_diesel
  names(by_trans_nrgfuel_ori[[2]]) <- 
    c("year", "常规公交", "快速公交", "农村客车")
  # 天然气消费量
  by_trans_nrgfuel_ori[[3]] <- 
    global_trans_gas[c("year", "公交合计", "出租车合计")]
  ...
  # 纯电动私家车：能耗总量为此前估算过的私家车每车能耗和上面活动水平部分计算的纯电动私家车数量之乘积
  ...
  trans_diremissum[["BY"]] <- 
    func_emissum(trans_nrgfuel[["BY"]], global_emisfac_df)
  ...
  ### Activity level ----
  # 服务业从业人口
  com_act[["BY"]] <- func_read_trans("厦门GDP和人口", "从业人口")
  ...
  ### Energy intensity ---- 
  com_nrgintst[["BY"]] <- 
    func_nrg_intst_ls(com_nrgsecfuel[["BY"]], com_act[["BY"]])
  ...
  by_tfres_ori_elecuse <- 
    func_ls2df(list(agri_nrgfuel[["BY"]], ind_nrgfuel[["BY"]], 
                    const_nrgfuel[["BY"]], trans_nrgfuel[["BY"]], 
                    com_nrgfuel[["BY"]], hh_nrgfuel[["BY"]]))
  ...
  # 合并活动水平
  tfres_act[["BY"]] <- 
    func_merge_2(list(tfres_act[["BY"]], by_tfres_ori_importelec))
  ...
  global_provelecgen_nrgsum <- 
    func_read_trans("福建省发电行业能源消费量和发电量")[
      c("year", "原煤", "柴油", "燃料油", "天然气")]
...
for (set_scalc in set_scalcs) {
  ...
  agri_act[[set_scalc]] <- func_interp_3(
    ...
    base = func_lastone(agri_act[["BY"]]$agri), "area", showplot = set_showplot)
  ...
  if (grepl("SLC", set_scalc)) {
    ...
    agri_nrgintst[[set_scalc]] <- func_interp_3( 
      ...
      base = func_lastone(agri_nrgintst[["BY"]]$diesel), 
      "diesel", showplot = set_showplot)
  ...
  if (grepl("SLC", set_scalc)) {
    ...
    agri_nrgintst[[set_scalc]]$electricity <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2045, 2060),
      scale = c(1.0, 1.01, 1.05, 1.13, 1.14, 1.15, 0.90), 
      base = func_lastone(agri_nrgintst[["BY"]]$electricity), 
      "electricity", showplot = set_showplot)$electricity
  } else if (grepl("WLC", set_scalc)) { 
    #### WLC ----
    agri_nrgintst[[set_scalc]]$electricity <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2045, 2060),
      scale = c(1.0, 1.01, 1.06, 1.15, 1.15, 1.16, 0.95), 
      base = func_lastone(agri_nrgintst[["BY"]]$electricity), 
      "electricity", showplot = set_showplot)$electricity
  } else { 
    #### BAU ----
    agri_nrgintst[[set_scalc]]$electricity <- func_interp_3(
      year = c(2019, 2020, 2025, 2045, 2060),
      scale = c(1.0, 1.01, 1.10, 1.10, 1.00), 
      base = func_lastone(agri_nrgintst[["BY"]]$electricity), 
      "electricity", showplot = set_showplot)$electricity
  }
  ...
  ind_ori_act_prop[[set_scalc]][, "石油及炼焦"] <- func_interp_2(
    year = c(2019, 2020, 2021, 2030, 2060), value = c(
      ...
      0.1, 0), showplot = set_showplot)$value
  ind_ori_act_prop[[set_scalc]][, "医药制造业"] <- func_interp_2(
    year = c(2019, 2020, 2021, 2030, 2060), value = c(
      ...
      2, 4), showplot = set_showplot)$value
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    ind_ori_act_prop[[set_scalc]][, "化学工业"] <- func_interp_2(
      year = c(2019, 2020, 2025, 2030, 2045, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"化学工业"[ind_ori_act_prop$BY$year == 2019], 
                     7.38, 6.30, 5.70, 1.00, 0.00), 
      showplot = set_showplot)$value
    ind_ori_act_prop[[set_scalc]][, "设备制造业"] <- func_interp_2(
      year = c(2019, 2020, 2025, 2030, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"设备制造业"[ind_ori_act_prop$BY$year == 2019], 
                     12.39, 13.8, 14.5, 19.0), 
      showplot = set_showplot)$value
    ind_ori_act_prop[[set_scalc]][, "电子电气制造业"] <- func_interp_2(
      year = c(2019, 2020, 2025, 2030, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"电子电气制造业"[
    ...
    ind_ori_act_prop[[set_scalc]][, "电子电气制造业"] <- func_interp_2(
      year = c(2019, 2020, 2025, 2030, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"电子电气制造业"[
          ind_ori_act_prop$BY$year == 2019], 
        48.97, 51.7, 53, 62), showplot = set_showplot)$value
  } else { 
    #### BAU ----
    ind_ori_act_prop[[set_scalc]][, "化学工业"] <- func_interp_2(
      year = c(2019, 2020, 2030, 2045, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"化学工业"[ind_ori_act_prop$BY$year == 2019], 
        7.38, 7, 3, 0), showplot = set_showplot)$value
    ind_ori_act_prop[[set_scalc]][, "设备制造业"] <- func_interp_2(
      year = c(2019, 2020, 2030, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"设备制造业"[ind_ori_act_prop$BY$year == 2019], 
        12.39, 13, 15), showplot = set_showplot)$value
    ind_ori_act_prop[[set_scalc]][, "电子电气制造业"] <- func_interp_2(
      year = c(2019, 2020, 2030, 2060), value = c(
        ind_ori_act_prop[["BY"]]$"电子电气制造业"[
          ind_ori_act_prop$BY$year == 2019], 
        48.97, 50, 55), showplot = set_showplot)$value
  }
      ...
      ind_nrgintst[[set_scalc]][[i]] <- cbind(
        ...
        sapply(global_ind_nrgclass[1:6], function(j) {
          func_interp_3(
            ...
            scale = c(1.0, 1.00, 1.00, 0.90, 0.89, 0.60), 
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]] <- cbind(
        ...
        sapply(global_ind_nrgclass[1:6], function(j) {
          func_interp_3(
            year = c(2019, 2020, 2025, 2030, 2035, 2060), 
            scale = c(1.0, 1.00, 1.00, 1.00, 1.00, 0.7), 
            base = func_lastone(ind_nrgintst[["BY"]][[i]][, j], 
                                zero.rm =  FALSE), 
            showplot = set_showplot)$value}))
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]] <- cbind(
        ...
        sapply(global_ind_nrgclass[1:6], function(j) {
          func_interp_3(
            year = c(2019, 2020, 2025, 2030, 2035, 2060), 
            scale = c(1.0, 1.00, 1.00, 1.00, 1.00, 0.80), 
            base = func_lastone(ind_nrgintst[["BY"]][[i]][, j], 
                                zero.rm =  FALSE), 
            showplot = set_showplot)$value}))
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]] <- cbind(
        ...
        sapply(global_ind_nrgclass[1:6], function(j) {
          func_interp_3(
            year = c(2019, 2020, 2025, 2030, 2035, 2060), 
            scale = c(1.0, 1.00, 1.00, 1.00, 1.00, 1.00), 
            base = func_lastone(ind_nrgintst[["BY"]][[i]][, j], 
                                zero.rm =  FALSE), 
            showplot = set_showplot)$value}))
    }
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "gas"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2030, 2035, 2060), 
        scale = c(1.0, 1.01, 1.03, 1.04, 1.05, 1.00), 
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "gas"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2030, 2035, 2060), 
        scale = c(1.0, 1.01, 1.03, 1.27, 1.30, 1.10), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "gas"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "gas"] <- func_interp_3(
        ...
        scale = c(1.0, 1.01, 1.03, 1.25, 1.24, 1.25, 1.40, 1.15), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "gas"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "gas"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2030, 2035, 2060), 
        scale = c(1.0, 1.01, 1.04, 1.37, 1.48, 1.20), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "gas"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "electricity"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2026, 2030, 2060), 
        scale = c(1, 1.0150, 1.04, 1.04, 1.06, 1.20), 
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "electricity"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2030, 2035, 2060), 
        scale = c(1, 1.0150, 1.04, 1.25, 1.27, 1.2), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "electricity"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "electricity"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2028, 2029, 2030, 2035, 2060), 
        scale = c(1, 1.0150, 1.04, 1.26, 1.25, 1.26, 1.30, 1.25), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "electricity"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
    ...
    for (i in global_ind_subsector) {
      ind_nrgintst[[set_scalc]][[i]][, "electricity"] <- func_interp_3(
        year = c(2019, 2020, 2025, 2030, 2035, 2060), 
        scale = c(1, 1.0150, 1.08, 1.35, 1.48, 1.3), 
        base = func_lastone(ind_nrgintst[["BY"]][[i]][, "electricity"], 
                            zero.rm =  FALSE), 
        showplot = set_showplot)$value
    }
  ...
  if (grepl("SLC", set_scalc)) {
    ...
    const_nrgintst[[set_scalc]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2040, 2060), 
      scale = c(1.0, 1.00, 0.75, 0.64, 0.40, 0.25), 
    ...
    const_nrgintst[[set_scalc]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2040, 2060), 
      scale = c(1.0, 1.00, 0.75, 0.65, 0.50, 0.40), 
      base = func_lastone(const_nrgintst[["BY"]]$electricity), 
      showplot = set_showplot, "electricity")
  } else { 
    #### BAU ----
    const_nrgintst[[set_scalc]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2040, 2060), 
      scale = c(1.0, 1.00, 0.70, 0.56, 0.50), 
      base = func_lastone(const_nrgintst[["BY"]]$electricity), 
      showplot = set_showplot, "electricity")
  }
    ...
    trans_act[[set_scalc]][, "私家车"] <- 
      c(func_lastone(trans_act[["BY"]][, "公路其他汽油"]), 
        func_curve_1(baseyear = 2020, basevalue = 1264236, 
                     maxyear = 2035, endyear = 2060, init_rate = 0.03, 
                     showplot = set_showplot)$value)
  } else { 
    #### NO SLC ----
    trans_act[[set_scalc]][, "私家车"] <- 
      c(func_lastone(trans_act[["BY"]][, "公路其他汽油"]), 
        func_curve_1(baseyear = 2020, basevalue = 1264236, 
                     maxyear = 2035, endyear = 2060, init_rate = 0.05, 
                     showplot = set_showplot)$value)
  }
  # 开始拆分：常规私家车和纯电动私家车比例
  ### BR.Elec4Gasoline ----
  if (grepl("PLUS", set_scalc)) { 
    ...
    trans_carprop_ls[[set_scalc]] <- func_interp_2(
      ...
      value = c(0.022, 0.023, 0.05, 0.16, 0.65, 0.75), 
    ...
    #### WLC ----
    trans_carprop_ls[[set_scalc]] <- func_interp_2(
      year = c(2019, 2020, 2025, 2028, 2030, 2050, 2060), 
      value = c(0.022, 0.023, 0.04, 0.07, 0.08, 0.50, 0.60), 
      showplot = set_showplot, "elec")
  } else { 
    #### BAU ----
    trans_carprop_ls[[set_scalc]] <- func_interp_2(
      year = c(2019, 2020, 2025, 2030, 2050, 2060), 
      value = c(0.022, 0.023, 0.03, 0.04, 0.40, 0.50), 
      showplot = set_showplot, "elec")
  }
      ...
      sapply(names(trans_nrgintst[["BY"]][[j]])[
        ...
        function(i) {
          func_interp_3(
            ...
            scale = c(1.0, 0.99, 0.96, 0.80), 
            base = func_lastone(trans_nrgintst[["BY"]][[j]][, i]), 
            showplot = set_showplot)$value}))
  }
  ...
  trans_nrgintst[[set_scalc]][["地铁"]] <- 
    func_interp_3(year = c(2019, 2025, 2035, 2060), 
                  scale = c(1.0, 0.97, 0.95, 0.90), 
                  base = trans_nrgintst$BY$"地铁"$electricity[
                    trans_nrgintst$BY$"地铁"$year == 2019], 
                  showplot = set_showplot, "electricity")
  ...
  trans_nrgintst[[set_scalc]][["公路其他汽油"]][, "gasoline"] <- func_interp_3(
    year = c(2019, 2020, 2025, 2030, 2035, 2060), 
    scale = c(1.0, 1.00, 1.03, 1.06, 1.06, 0.7), 
    base = func_lastone(trans_nrgintst[["BY"]][["公路其他汽油"]]$gasoline), 
    showplot = set_showplot)$value
  ...
  trans_nrgintst[[set_scalc]][["纯电动私家车"]][, "electricity"] <- 
    func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2040, 2060), 
      scale = c(1.0, 1.00, 1.02, 1.03, 0.8, 0.7), 
      base = func_lastone(
        trans_nrgintst[["BY"]][["纯电动私家车"]]$electricity), 
      showplot = set_showplot)$value
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    trans_nrgintst[[set_scalc]][["水路客运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.90, 0.68, 0.68, 0.65), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$diesel), 
      showplot = set_showplot, "diesel")
    trans_nrgintst[[set_scalc]][["水路客运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.92, 0.68, 0.68, 0.65), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$residual), 
      "residual", showplot = set_showplot)$residual
  } else if (grepl("SLC", set_scalc)) { 
    #### SLC ----
    trans_nrgintst[[set_scalc]][["水路客运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2060), 
      scale = c(1.0, 1.00, 0.90, 0.89, 0.80), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$diesel), 
      showplot = set_showplot, "diesel")              
    trans_nrgintst[[set_scalc]][["水路客运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2060), 
      scale = c(1.0, 1.00, 0.92, 0.90, 0.80), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$residual), 
      showplot = set_showplot, "residual"
    )$residual
  } else if (grepl("WLC", set_scalc)) { 
    #### WLC ----
    trans_nrgintst[[set_scalc]][["水路客运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.90, 0.95, 0.92, 0.92, 0.90), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$diesel), 
      showplot = set_showplot, "diesel")
    trans_nrgintst[[set_scalc]][["水路客运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2030, 2060), 
      scale = c(1.0, 1.00, 0.92, 0.94, 0.90, 0.90), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路客运"]]$residual), 
      showplot = set_showplot, "residual")$residual
  } else { 
    #### BAU ----
    trans_nrgintst[[set_scalc]][["水路客运"]] <- func_interp_3(
      year = c(2019, 2060), 
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    trans_nrgintst[[set_scalc]][["水路货运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.95, 0.80, 0.79, 0.70), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$diesel), 
      showplot = set_showplot,"diesel")
    trans_nrgintst[[set_scalc]][["水路货运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.97, 0.80, 0.79, 0.70), 
    ...
    trans_nrgintst[[set_scalc]][["水路货运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2060), 
      scale = c(1.0, 1.00, 0.95, 0.93, 0.80), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$diesel),
      "diesel", showplot = set_showplot)
    # 燃料油：基于历史数据和比率
    trans_nrgintst[[set_scalc]][["水路货运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2060), 
      scale = c(1.0, 1.00, 0.97, 0.92, 0.80), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$residual),
      "residual", showplot = set_showplot)$residual
  } else if (grepl("WLC", set_scalc)) { 
    #### WLC ----
    trans_nrgintst[[set_scalc]][["水路货运"]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.95, 1.00, 1.00, 1.00, 0.9), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$diesel),
      "diesel", showplot = set_showplot)
    trans_nrgintst[[set_scalc]][["水路货运"]]$residual <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2030, 2035, 2060), 
      scale = c(1.0, 1.00, 0.97, 1.00, 1.00, 1.00, 0.90), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$residual),
      "residual", showplot = set_showplot)$residual
  } else { 
    #### BAU ----
    trans_nrgintst[[set_scalc]][["水路货运"]] <- func_interp_3(
      year = c(2019, 2060), scale = c(1, 1), 
      base = func_lastone(trans_nrgintst[["BY"]][["水路货运"]]$diesel),
      "diesel", showplot = set_showplot)
    trans_nrgintst[[set_scalc]][["水路货运"]]$residual <- func_interp_3(
      year = c(2019, 2060), scale = c(1, 1), 
  ...
  trans_nrgintst[[set_scalc]][["航空"]] <- func_interp_3(
    ...
    base = func_lastone(trans_nrgintst[["BY"]][["航空"]]$kerosene), 
    "kerosene", showplot = set_showplot)
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    com_nrgintst[[set_scalc]][[1]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.01, 1.04, 1.03, 1.06, 0.80), 
    ...
    com_nrgintst[[set_scalc]][[1]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2029, 2030, 2035, 2060), 
      scale = c(1.0, 1.01, 1.05, 1.07, 1.07, 1.09, 1.10, 0.95), 
      base = func_lastone(com_nrgintst[["BY"]]$electricity$electricity), 
      "electricity", showplot = set_showplot)
  } else { 
    #### BAU ----
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    com_nrgintst[[set_scalc]][[2]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.03, 1.11, 1.10, 1.10, 0.80), 
    ...
    com_nrgintst[[set_scalc]][[2]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2028, 2030, 2035, 2060), 
      scale = c(1.0, 1.03, 1.12, 1.25, 1.24, 1.30, 1.00), 
      base = func_lastone(com_nrgintst[["BY"]][[2]]$lpg), 
      "lpg", showplot = set_showplot)
  } else { 
    #### BAU ----
    com_nrgintst[[set_scalc]][[2]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.03, 1.13, 1.36, 1.44, 1.20), 
      base = func_lastone(com_nrgintst[["BY"]][[2]]$lpg), 
      "lpg", showplot = set_showplot)
  }
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    com_nrgintst[[set_scalc]][[2]]$gas <- func_interp_3(
      ...
      scale = c(1.0, 1.06, 1.15, 1.33, 1.33, 1.36, 1.00), 
      base = func_lastone(com_nrgintst[["BY"]][[2]]$gas), 
      "gas", showplot = set_showplot)$gas
  } else { 
    #### BAU ----
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    com_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      yearsubs = list(c(2019, 2020, 2025, 2030, 2050, 2060), 
                      c(2019, 2020, 2025, 2030, 2050, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.30, 0.70, 1, 1), 
                      c(0.00, 0.05, 0.30, 0.70, 1, 1)), 
      alterscales = list(0.8, 0.8))
  } else if (grepl("WLC", set_scalc)) { 
    #### WLC ----
    com_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      namenrgoris = list("lpg", "gas"), 
      namenrgsubs = list("electricity", "electricity"), 
      yearsubs = list(c(2019, 2020, 2025, 2035, 2053, 2060), 
                      c(2019, 2020, 2025, 2035, 2053, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.25, 0.60, 1.00, 1.00), 
                      c(0.00, 0.05, 0.25, 0.60, 1.00, 1.00)), 
      alterscales = list(0.8, 0.8))
  } else { 
    #### BAU ----
    com_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      namenrgoris = list("lpg", "gas"), 
      namenrgsubs = list("electricity", "electricity"), 
      yearsubs = list(c(2019, 2020, 2040, 2055, 2060), 
                      c(2019, 2020, 2040, 2055, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.60, 1, 1), 
                      c(0.00, 0.05, 0.60, 1, 1)), 
      alterscales = list(0.8, 0.8))
  }
  ...
  if (grepl("PLUS", set_scalc)) { 
    ...
    hh_nrgintst[[set_scalc]][[1]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2030, 2035, 2060), 
      scale = c(1.0, 1.01, 1.15, 1.23, 1.34, 1.40), 
    ...
    hh_nrgintst[[set_scalc]][[1]] <- func_interp_3(
      year = c(2019, 2020, 2025, 2029, 2030, 2035, 2060), 
      scale = c(1.0, 1.01, 1.17, 1.28, 1.29, 1.44, 1.50), 
      base = func_lastone(hh_nrgintst[["BY"]][["household"]][, "electricity"]), 
      showplot = set_showplot)
  } else { 
    #### BAU ----
    hh_nrgintst[[set_scalc]][[1]] <- func_interp_3(
      ...
      scale = c(1.0, 1.01, 1.18, 1.31, 1.32, 1.48, 1.60), 
      base = func_lastone(hh_nrgintst[["BY"]][["household"]][, "electricity"]), 
      showplot = set_showplot)
  }
  ...
  hh_nrgintst[[set_scalc]][[1]]$rawcoal <- func_interp_2(
    ...
    value = c(func_lastone(hh_nrgintst$BY$household$rawcoal), 0, 0), 
    showplot = set_showplot)$value
  ...
  if (grepl("SLC", set_scalc)) { 
    ...
    hh_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      yearsubs = list(c(2019, 2020, 2030, 2050, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.70, 0.90, 0.90)), 
    ...
    hh_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      namenrgoris = list("lpg"), 
      namenrgsubs = list("electricity"), 
      yearsubs = list(c(2019, 2020, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.60)), 
      alterscales = list(0.8))
  } else { 
    #### BAU ----
    hh_nrgintst[[set_scalc]][[2]] <- func_nrgsub(
      ...
      namenrgsubs = list("electricity"), 
      yearsubs = list(c(2019, 2020, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.50)), 
      alterscales = list(0.8))
  }
  ...
  if (grepl("SLC", set_scalc)) { 
    ...
    hh_nrgintst[[set_scalc]][[3]] <- func_nrgsub(
      ...
      namenrgsubs = list("electricity"), 
      yearsubs = list(c(2019, 2020, 2030, 2050, 2060)), 
      propsubs = list(c(0.00, 0.05, 0.60, 0.90, 0.90)), 
      alterscales = list(0.8))
  } else if (grepl("WLC", set_scalc)) { 
    #### WLC ----
    hh_nrgintst[[set_scalc]][[3]] <- func_nrgsub(
  ...
  tf_nrgintst[[set_scalc]] <- cbind(
    data.frame(year = c(2019: 2060)), 
    sapply(
      names(tf_nrgintst[["BY"]])[names(tf_nrgintst[["BY"]]) %in% 
                                   "year" == FALSE], 
      function(i) {
  ...
  if (grepl("KEEPCOAL", set_scalc)) {
    ...
    tfres_act[[set_scalc]] <- 
      func_merge_2(list(
        ...
        func_interp_3(
          year = c(2019, 2030, 2040, 2060), 
          scale = c(1.0, 1.00, 0.50, 0.50), 
          base = tfres_act[["BY"]]$elecgen_thrm[tfres_act[["BY"]]$year == 2019],  
          "elecgen_thrm", showplot = set_showplot)))
  }
  # 假设未来天然气发电量保持不变
  tfres_act[[set_scalc]]$elecgen_thrm[which(
    tfres_act[[set_scalc]]$year > 2019)] <- 
    tfres_act[[set_scalc]]$elecgen_thrm[which(
      tfres_act[[set_scalc]]$year > 2019)] + 120000
  ...
  tfres_provelecgenstr <- data.frame(
    ...
    thrm_prop = func_interp_2(
      ...
      value = c(0.546504985, 0.291973506, 0.249449437, 0.161468824, 0), 
      showplot = set_showplot)$value)
...
# 输出关键指标
{
  ...
  for (i in set_scalcs) {
    ...
    idx_all[[i]] <- data.frame(
      ...
      单位GDP能耗 = func_cross(
        tot_nrgsumce[[i]], 
        prj_global_gdp[c("year", "GDP")],"rate")$energyconsump
    )
  }
...
## For report ----
{
  ...
  report_tab2 <- rbind(
    ...
    report_tab2[which(report_tab2$scenario == set_starget), ]
  )
